...
def prepare_next_question(self):
    """Prepara la prossima domanda per l'esercizio corrente, distinguendo tra esercizi testuali e con immagini."""
    if self.current_exercise:
        print(f"[GlobalStateManager] Preparing next question for {self.current_exercise}.")
        try:
            # Verifica se l'esercizio richiede immagini
            if self.current_exercise.get("exerciseWithImage"):
                print("[GlobalStateManager] Image-based exercise detected.")
                # Genera una domanda basata su immagini
                ...
                print("[GlobalStateManager] Text-based exercise detected.")
                # Genera una domanda testuale
                self.current_question = {
                    "expected_response": "venire",
                    "question": "(tu - venire) …………………….. sul binario 3, per favore.",
                }
            return self.current_question
        except Exception as e:
            print(f"[GlobalStateManager] Error preparing question: {e}")
            self.current_question = {
                "expected_response": "",
                "question": "Errore nel caricamento dell'esercizio. Prova con un altro esercizio o riprova più tardi."
            }
            return self.current_question
    else:
